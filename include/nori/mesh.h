...
struct Intersection {
    ...
    bool hasHit() const { return mesh != nullptr; }
};
...
public:
    ...
    /// Return the surface area of mesh
    float surfaceArea() const { return dpdf.getNormalization(); }
    /// Return the surface area of the given triangle
    float surfaceArea(uint32_t index) const;

    /// Return total surface area
    float meshArea() const { return dpdf.getSum(); }
    ...
    /**
     * \brief Get uniformly samples positions on the mesh
     * \param sampler
     *      sampling pointer to get a square sample 2d
     * \param n
     *      the interpolated surface normal n at point p calculated from per-vertex normals(if provided, else face normal)
     * \param pdf
     *      the pdf value corresponding to the sample points
     * \return
     *      sample point3f
     * */
    Point3f squareToUniformMesh(Sampler *sampler, Normal3f &n, float &pdf) const;

    float squareToUniformMeshPDF() const;
protected:
...
protected:
    ...
    DiscretePDF dpdf;               ///mesh pdf for sampling
};
